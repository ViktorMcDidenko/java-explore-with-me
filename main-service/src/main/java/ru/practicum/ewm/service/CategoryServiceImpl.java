...
import ru.practicum.ewm.model.CaregoryMapper;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    private final CaregoryMapper mapper;

    @Override
    public CategoryDto add(CategoryDto categoryDto) {
        Category category = mapper.categoryDtoToCategory(categoryDto);
        Category savedResult = repository.save(category);
        return mapper.categoryToCategoryDto(savedResult);
    }
    ...
    public CategoryDto change(long id, CategoryDto categoryDto) {
        ...
        Category updatedCategory = repository.save(savedCategory); //если не сработает, сначала удалить, потом сохранить заново, без приблуд выше
